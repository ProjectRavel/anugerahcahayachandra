...
import InboundOrderActions from '@/components/dashboard/gudang/InboundOrderActions'
...
export default async function InboundPage() {
  ...
  const canCreateInbound = currentRole === 'ADMIN_GUDANG'
  ...
  const [{ data: rows }, { data: statusRows }, { data: productRows }] = await Promise.all([
    supabase
      ...
      .limit(50),
    supabase.from('orders').select('status').eq('type', 'INBOUND'),
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Penerimaan (Inbound)</h1>
        <p className="text-sm text-slate-500 mt-1 max-w-2xl">
          Pantau dan kelola order inbound (penerimaan barang) dari supplier.
        </p>
      </div>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <StatsCard title="Processing" value={processing} subtitle="diproses" accent="indigo" />
          <StatsCard title="Completed" value={completed} subtitle="selesai" accent="emerald" />
          <StatsCard
            title="Nilai Inbound"
      ...
      {canCreateInbound && (
        <section>
          ...
          <CreateOrderForm type="INBOUND" products={products} />
        </section>
      )}
          ...
          <div className="overflow-x-auto">
            ...
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500">Aksi</th>
              </tr>
            </thead>
                ...
                ordersWithSummary.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/70">
                    ...
                    <td className="px-4 py-3 text-xs text-slate-700 text-right whitespace-nowrap">
                      {canCreateInbound ? (
                        <InboundOrderActions orderId={order.id} status={order.status} />
                      ) : (
